...
func InsertUser(db *sqlx.DB, req *users.Register, isAdmin bool) IInsertUser {
...
func newCustomer(db *sqlx.DB, req *users.Register) IInsertUser {
	return &customer{
		userReq: &userReq{
			req: req,
			db:  db,
...
func newAdmin(db *sqlx.DB, req *users.Register) IInsertUser {
	return &admin{
		userReq: &userReq{
			req: req,
			db:  db,
